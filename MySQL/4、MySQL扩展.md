...
```sh
mysqlshow -uroot -p123456 --count # 统计每个数据库中每个表的数量，及行数
mysqlshow -uroot -p123456 test --count # 统计test数据库中每个表的数量，及行数
mysqlshow -uroot -p123456 test book --count # 统计test数据库中book表的字段数，及行数
mysqlshow -uroot -p123456 test book id --count # 统计test数据库中book表的id字段的信息
```
...
```
语法：
	mysqldump [options] 数据库名称 [表1...] # 可以指定备份数据库中的哪些表
	mysqldump [options] --database/-B db1 [db2 db3...] # 可以指定备份哪些数据库
	mysqldump [options] --all-databases/-A # 备份所有的数据库
连接选项：
    -u, --user=name 指定用户名
    -p, --password[=name] 指定密码
    -h, --host=name 指定服务器ip或域名
...
1. 备份stu数据库
   ...
   ```sh
   mysqldump -uroot -p123456 stu > stu.sql
   ```

   可以直接打开db01.sql，备份出来的数据包含：
...
2. 备份stu数据库中的表数据，不备份表结构(-t)

   ```sh
   mysqldump -uroot -p123456 -t stu > stu.sql
   ```

   打开 db02.sql，来查看备份的数据，只有insert语句，没有备份表结构。

3. 将stu数据库的表的表结构与数据分开备份(-T)

   ```sh
   mysqldump -uroot -p123456 -T /root stu score
   ```

   执行上述指令会出错，只有表结构完成了备份，数据不能完成备份，原因是因为我们所指定的数据存放目录/root，MySQL认为是不安全的，需要存储在MySQL信任的目录下。那么，哪个目录才是MySQL信任的目录呢，可以查看一下系统变量 secure_file_priv。执行结果如下：

   ![image-20221013150159704](https://raw.githubusercontent.com/zsc-dot/pic/master/img/Git/image-20221013150159704.png)

   ```sh
   mysqldump -uroot -p123456 -T /var/lib/mysql-files stu score
   ```

   需要注意表数据文件，并不是记录一条条的insert语句，而是按照一定的格式记录表结构中的数据。
...
1. mysqlimport
   ...
   ```
   语法：
   	mysqlimport [options] 数据库名 文件1[文件2...]
   ```
...
2. source
   ...
   如果要导入sql文件，可以使用mysql中的source 指令：
...
1. mysql
   ...
   MySQL客户端工具，-e执行SQL并退出，编写脚本时会用到

2. mysqladmin

   不需要进入mysql即可创建、删除数据库
